...
const clientModule = {
    ...
    getters: {
        ...
        WHOLESALE_CLIENT: (state) => (id) =>
            state.wholesale_clients.find((c) => c.id === id),
    ...
    mutations: {
        ...
        editWholesaleClient(state, payload) {
            state.wholesale_clients = state.wholesale_clients.map((client) => {
    ...
    actions: {
        ...
        async [ACTIONS.GET_PARTNERS]({ commit }) {
            const {
                data: { data },
            } = await axiosClient.get(`/clients?is_partner=1`);
            await commit(MUTATIONS.SET_CLIENTS, data);
        },
        async [ACTIONS.GET_WHOLESALE_CLIENTS]({ commit }) {
            const {
                data: { data },
            } = await axiosClient.get('/clients?wholesale=1');
        ...
        async [ACTIONS.ADD_BARTER_BALANCE]({ commit }, payload) {
            const { data } = await axiosClient.post('/v3/barter', payload);
            await commit(MUTATIONS.EDIT_CLIENT, data.client);
        },
        ...
        async [ACTIONS.EDIT_WHOLESALE_CLIENT]({ commit }, payload) {
            const {
                data: { data },
            } = await axiosClient.patch(`/clients/${payload.id}`, payload);
