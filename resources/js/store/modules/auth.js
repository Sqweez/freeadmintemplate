...
const authModule = {
    ...
    getters: {
        ...
        IS_OPT_MANAGER: (state) => state.user && +state.user.role_id === 12,
        CAN_SALE: (state, getters) =>
            getters.IS_ADMIN ||
            getters.IS_SELLER ||
            getters.IS_BOSS ||
        ...
        IS_MALOY: (state, getters) =>
            !!(getters.IS_MODERATOR && state.user.login === 'maloy'),
        CURRENT_ROLE: (state, getters) => {
            const roles = {
                ...
                opt_manager: getters.IS_OPT_MANAGER,
            };
