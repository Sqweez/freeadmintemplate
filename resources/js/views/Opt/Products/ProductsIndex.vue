<template>
   <i-card-page title="Оптовые товары">
       <div>
           <v-btn color="success" @click="$router.push('/opt/products/create')">
               Добавить товар
           </v-btn>
       </div>
       <v-row>
               ...
               <v-row>
                   ...
                   <v-col cols="12" xl="4">
                       <v-select
                           ...
                           item-value="id"
                           label="Склад"
                           :disabled="!(is_admin || IS_BOSS) || true"
                       />
                   </v-col>
                   <v-col cols="12" xl="2">
                       <v-checkbox
               ...
               <v-data-table
                   ...
                   <template v-slot:item.actions="{ item }">
                       ...
                       <div class="actions-products__container" v-if="IS_MODERATOR">
                           <v-btn color="warning" @click="$router.push(`/opt/products/edit/${item.id}`)">
                               Товар
                               <v-icon>mdi-pencil</v-icon>
                           </v-btn>
                       </div>
                       <div class="actions-products__container" v-if="IS_OPT_MANAGER">
                           ...
                           <v-btn color="success" @click="$router.push(`/opt/products/create?id=${item.id}`)">
                               Новый товар
                               <v-icon>mdi-plus</v-icon>
                           </v-btn>
    ...
    computed: {
        ...
        stores() {
            return [{
                name: 'Все',
                id: -1
            }, ...this.$store.getters.wholesale_stores];
        },
        categories() {
            return [{
                id: -1,
        ...
        headers() {
            ...
            if (this.is_admin || this.IS_BOSS || this.IS_SENIOR_SELLER || this.IS_MODERATOR || this.IS_FRANCHISE || this.IS_OPT_MANAGER) {
                headers.unshift({
                    value: 'actions',
                    text: 'Действие',
                    sortable: false
