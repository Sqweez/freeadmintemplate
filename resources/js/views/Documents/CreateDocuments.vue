    ...
    <div>
        ...
        <v-card v-if="!emptyCart">
            ...
            <v-card-text style="padding: 0;">
                <v-btn depressed color="error" class="top-button ml-6" @click="showWaybillModal = true;">
                    Сформировать накладную
                </v-btn>
                <v-btn depressed color="error" class="top-button ml-2" @click="showInvoiceModal = true;">
                    Сформировать счет-фактуру
                </v-btn>
                <v-btn depressed color="error" class="top-button ml-2" @click="showInvoicePaymentModal = true;">
                    Сформировать счет на оплату
                </v-btn>
                <v-btn depressed color="error" class="top-button ml-2" @click="showProductCheckModal = true;">
                ...
                <v-simple-table v-slot:default>
                    <template>
                        ...
                        <tr v-for="(item, index) of cart" :key="`product-id-${item.uuid}`">
                            ...
                            <td>{{
                                    item.product_price * item.count - (Math.max(discountPercent, item.discount) / 100 *
